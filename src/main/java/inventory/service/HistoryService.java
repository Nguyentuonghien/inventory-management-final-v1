...
public class HistoryService {
	...
	public List<History> getAllHistory(History history, Paging page) {
		StringBuilder queryStr = new StringBuilder();
		Map<String, Object> mapParams = new HashMap<>();
		
		// kiểm tra form search không rỗng và các trường liên quan đến history và productInfo(code,category) mà ta search không rỗng bên history.jsp
		if (history != null) {
			...
			// mặc định là search all(type==0). ta sẽ search theo hóa đơn nhập hàng(type==1) hoặc xuất hàng(type==2)
			if (history.getType() != 0) { 
				queryStr.append(" and model.type=:type");
				mapParams.put("type", history.getType());
			}
		}
		return historyDAO.findAll(queryStr.toString(), mapParams, page);
	}
	
	// lưu dạng vết về nhập hàng, xuất hàng, thêm, sửa, xóa hóa đơn 
